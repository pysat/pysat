...
pysat release v3.0 introduced some backwards incompatible changes from
...
``os.path.join(platform, name, tag, inst_id)``. Thus, by default, a pysat v3.0+
install will generally not find all existing data files that were managed by
pysat v2.x.
