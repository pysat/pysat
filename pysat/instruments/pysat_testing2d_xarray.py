...
def load(fnames, tag=None, sat_id=None):
    ...
    yr = 2009
    #parts = os.path.split(fnames[0])[-1].split('-')
    day = fnames.split('_')[-1].split('.')[-2]
    
    from datetime import datetime
    dateObj = datetime.strptime(str(yr)+'-'+day,'%Y-%j')
    date = pysat.datetime(yr, dateObj.month, dateObj.day)
    
    ds_disk = xr.open_dataset(fnames)
...
data, meta = load(r'C:\Data\Gayatri\US\UTD\SORTIE\UCol Summer Internship\Data\tiegcm_icon_merg2.0_totTgcm.s_149.nc')
print("Length:", len(data.variables))
print("meta\n", meta)
print("Data: \n", data)
print("---------------------")
#print(data.keys)
#print(data.variables['profiles'])
#print(len(data.variables['profiles']))

print(data.variables['latitude'])
print(len(data.variables['latitude']))
